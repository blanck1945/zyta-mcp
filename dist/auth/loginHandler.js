...
import { fetchCurrentUser, getBaseUrl, getTokenFilePath, setToken, validateToken, } from "../session.js";
...
export const loginInputSchema = z.object({
    ...
    email: z
        .string()
        .optional()
        .describe("Email de Zyta. Con password hace POST /auth/login (recomendado si /mcp-device no está desplegado)."),
    password: z
        .string()
        .optional()
...
export async function executeLogin(args, options = {}) {
    ...
    try {
        ...
        if (email || password !== undefined) {
            if (!email || !password) {
                return toolError(new Error("Para login con credenciales indicá email y password juntos."));
            }
