...
export async function startDeviceAuthorization(opts) {
    ...
    try {
        ...
        throw new Error(`device-authorization: respuesta no JSON (${res.status}): ${text.slice(0, 300)}`);
    }
    if (!res.ok) {
    ...
    const verificationUriComplete = readField(data, "verificationUriComplete", "verification_uri_complete");
    ...
    return {
        ...
        userCode,
        verificationUri,
        verificationUriComplete: typeof verificationUriComplete === "string"
...
function parseTokenResponse(data) {
    const token = readField(data, "accessToken", "access_token");
    if (typeof token === "string" && token.length > 0)
        return token;
    throw new Error("Respuesta token sin accessToken / access_token");
}
export async function pollDeviceUntilAccessToken(opts, session) {
    ...
    while (Date.now() < deadline) {
        ...
        if (oauthErr === "authorization_pending" ||
